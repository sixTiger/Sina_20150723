...
@implementation XXBStatusToolBar
...
- (void)setStatus:(XXBStatus *)status
{
    ...
    // 设置转发按钮的标题
    [self setupButton:self.retweetBtn count:status.reposts_count originalTitle:@"转发"];
    
    // 设置评论按钮的标题
    [self setupButton:self.commentBtn count:status.comments_count originalTitle:@"评论"];
    
    // 设置赞按钮的标题
    [self setupButton:self.unlikeBtn count:status.attitudes_count originalTitle:@"赞"];
}
